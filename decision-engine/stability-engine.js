(function createStabilityEngine(root) {
  ...
  function evaluate({ ticker, horizon, candidateAction, edge, confidence, materialChangeReasons = [], profile = {}, technical = {} } = {}) {
    ...
    if (previous && !material && previous.previousAction !== candidateAction) {
    ...
    const next = {
      previousAction: finalAction, previousEdge: edge, previousConfidence: confidence,
    ...
    return { finalAction, heldPrevious, material, materialChangeReasons, score: Math.round(score), firstObservation: !previous, persistence: technical.signalPersistence || null, previous: previous ? { previousAction: previous.previousAction, previousEdge: previous.previousEdge, previousConfidence: previous.previousConfidence, lastMaterialChange: previous.lastMaterialChange } : null, lastMaterialChange: next.lastMaterialChange };
  }

  engine.stability = Object.freeze({ evaluate, clear: () => cache.clear(), _cache: cache });
}(globalThis));
